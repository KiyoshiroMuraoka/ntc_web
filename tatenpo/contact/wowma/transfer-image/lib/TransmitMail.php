...
class TransmitMail
{
    ...
    public function setTemplateAndSendMail()
    {
        ...
        if ($this->page_name === 'deny') {
            ...
            // HTML を表示
            echo str_replace('<!--##AcceptID##-->', $acceptId, $this->tpl->fetch($this->config['tpl_error']));
            ...
            // 受付番号
            $acceptId = 'W'.date("Ym"). str_pad(mt_rand(1, 9999), '0', STR_PAD_LEFT);
            // メール送信
            $this->sendMail(false, $acceptId);

            // 自動返信メール送信
            if ($this->config['auto_reply'] && !empty($this->post[$this->config['auto_reply_email']])) {
                $this->sendMail(true, $acceptId);
            }
            ...
            if ($this->global_errors) {
                ...
                // 完了画面を表示
                echo str_replace('<!--##AcceptID##-->', $acceptId, $this->tpl->fetch($this->config['tpl_finish']));
            }
        } elseif ($this->page_name === 'confirm') {
            // 確認画面

            // HTML を表示
            echo str_replace('<!--##AcceptID##-->', $acceptId, $this->tpl->fetch($this->config['tpl_confirm']));
        } else {
            // 入力画面 or 入力エラー画面
            ...
            // HTMLを表示
            echo str_replace('<!--##AcceptID##-->', $acceptId, $this->tpl->fetch($this->config['tpl_input']));
        }
    }
     ...
     * @params array
     */
    public function sendMail($is_auto_reply = false, $acId = "none")
    {
        ...
        $to_subject = str_replace("<AcceptID>", $acId, $to_subject);
        $body = str_replace("<!--##AcceptID##-->", $acId, $body);
        $body = str_replace("<!--##Date##-->", date('Y/m/d（D）H:i:s'), $body);
